// NOTE (environment-forced location): the canonical gauge lives at
// client/src/components/patient/AutonomicBalanceGauge.tsx, but that directory is
// read-only in this workspace, so the SDNN/LF-HF overlap fix could not be applied
// in place. This is a drop-in copy with that fix; ReportDashboard renders it via
// PatientPortalTwoColumn. RECONCILE: when patient/ becomes writable, fold the
// `containerType`/`fs()` changes below back into the original file and delete this
// shim (the only change vs. the original is label font scaling — no data logic).
...
export function AutonomicBalanceGauge({
  ...
  const activeLeft = balanceAssessed
    ? Math.round((sPct / 100) * arcTicksLeft.length)
    : 0;
  ...
  // Overlay labels use fixed px over a fluid SVG; scale them with the gauge
  // width (container query units) so proportions — and the spacing between the
  // Sympathetic/Parasympathetic %, SDNN and LF/HF readouts — are preserved at
  // any column width, eliminating overlap in the narrower two-column layout.
  // Reference width = max-w-xl (576px) design size.
  const fs = (px: number, min: number) =>
    `clamp(${min}px, ${((px * 100) / 576).toFixed(2)}cqw, ${px}px)`;

  return (
    <div
      className="relative w-full max-w-xl mx-auto select-none"
      style={{ aspectRatio: "1 / 1.05", containerType: "inline-size" }}
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        >
          <div
            ...
            style={{ color: "hsl(185 70% 70%)", fontSize: fs(10, 8), letterSpacing: "0.22em" }}
          >
            HRV · RMSSD
          </div>
          <div
          ...
          >
            {rmssdText}
            {rmssdText !== "—" && (
              <span className="ml-1" style={{ fontSize: fs(12, 9), color: "hsl(185 50% 75%)" }}>
                ms
              </span>
            )}
          </div>
        ...
        >
          ...
          >
            {sdnnText}
            {sdnnText !== "—" && (
              <span className="ml-1" style={{ fontSize: fs(11, 9), color: "hsl(18 50% 75%)" }}>
                ms
              </span>
            )}
          </div>
