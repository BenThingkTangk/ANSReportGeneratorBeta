...
-- Migration: 0002_seed_admins_and_sources
-- Seeds: admin user_roles + all 13 knowledge sources
-- ============================================================

-- ============================================================
-- 1. Seed admin roles (idempotent — safe to re-run)
-- ============================================================
DO $$
DECLARE
  v_ben_id  uuid;
  ...
  v_so_id   uuid;
BEGIN
  -- super_admin: ben.oleary@thingktangk.com
  SELECT id INTO v_ben_id FROM auth.users WHERE email = 'ben.oleary@thingktangk.com' LIMIT 1;
  IF v_ben_id IS NOT NULL THEN
    INSERT INTO public.user_roles (user_id, role)
    VALUES (v_ben_id, 'super_admin')
    ON CONFLICT (user_id) DO UPDATE SET role = 'super_admin', updated_at = now();
  END IF;

  -- clinical_admin: jcolombo@physiops.com
  SELECT id INTO v_jc_id FROM auth.users WHERE email = 'jcolombo@physiops.com' LIMIT 1;
  IF v_jc_id IS NOT NULL THEN
    INSERT INTO public.user_roles (user_id, role)
    VALUES (v_jc_id, 'clinical_admin')
    ON CONFLICT (user_id) DO UPDATE SET role = 'clinical_admin', updated_at = now();
  END IF;

  -- clinical_admin: soleary@physiops.com
  SELECT id INTO v_so_id FROM auth.users WHERE email = 'soleary@physiops.com' LIMIT 1;
  IF v_so_id IS NOT NULL THEN
    INSERT INTO public.user_roles (user_id, role)
    VALUES (v_so_id, 'clinical_admin')
    ON CONFLICT (user_id) DO UPDATE SET role = 'clinical_admin', updated_at = now();
  END IF;
END;
$$;
...
-- ============================================================
-- 2. Seed knowledge sources (idempotent — keyed on doi/title)
-- ============================================================
