...
const RECOGNIZERS: Recognizer[] = [
  {
    ...
    unit: null,
    patterns: [
      new RegExp(String.raw`sympathovagal\s+balance[^\d-]{0,20}${NUM}`, "i"),
      new RegExp(String.raw`\bSB\b[^\d-]{0,12}${NUM}`, "i"),
      new RegExp(String.raw`LFa\s*/\s*RFa[^\d-]{0,12}${NUM}`, "i"),
    ],
  },
  {
    key: "LFHF",
    label: "LF/HF ratio",
    unit: null,
    patterns: [new RegExp(String.raw`LF\s*/\s*HF[^\d-]{0,12}${NUM}`, "i")],
  },
  {
    key: "LFa",
    label: "LFa (sympathetic)",
    unit: "bpm²",
    patterns: [new RegExp(String.raw`\bLFa\b[^\d-]{0,20}${NUM}`, "i")],
  },
  {
    key: "RFa",
    label: "RFa (parasympathetic)",
    unit: "bpm²",
    patterns: [new RegExp(String.raw`\bRFa\b[^\d-]{0,20}${NUM}`, "i")],
  },
  {
    key: "HRV_SDNN",
    label: "SDNN",
    unit: "ms",
    patterns: [new RegExp(String.raw`\bSDNN\b[^\d-]{0,15}${NUM}`, "i")],
  },
  {
    key: "HRV_RMSSD",
    label: "RMSSD",
    unit: "ms",
    patterns: [new RegExp(String.raw`\bRMSSD\b[^\d-]{0,15}${NUM}`, "i")],
  },
  {
    key: "SBP",
    label: "Systolic BP",
    unit: "mmHg",
    patterns: [
      new RegExp(String.raw`systolic[^\d-]{0,15}${NUM}`, "i"),
      new RegExp(String.raw`\bSBP\b[^\d-]{0,12}${NUM}`, "i"),
    ],
  },
  {
    key: "DBP",
    ...
    unit: "mmHg",
    patterns: [
      new RegExp(String.raw`diastolic[^\d-]{0,15}${NUM}`, "i"),
      new RegExp(String.raw`\bDBP\b[^\d-]{0,12}${NUM}`, "i"),
    ],
  },
  {
    key: "eiRatio",
    label: "E/I ratio",
    unit: null,
    patterns: [new RegExp(String.raw`E\s*/\s*I\s*ratio[^\d-]{0,12}${NUM}`, "i")],
  },
  {
    key: "valsalvaRatio",
    label: "Valsalva ratio",
    unit: null,
    patterns: [new RegExp(String.raw`valsalva\s*ratio[^\d-]{0,12}${NUM}`, "i")],
  },
];
